...
from detection_engine import DetectionEngine  # Import to access signature rules
...
class AlertSystem:
    def __init__(self, db_config):
        ...
        self.cursor = None
        self._connect_db()
        self._load_and_insert_alert_rules()  # Automate alert rule loading
    ...
    async def _insert_alert_rule(self, name, description, condition, severity_level, category=None):
        try:
            query = """
                INSERT INTO alert_rules (name, description, condition, severity_level_id, category)
                VALUES (%s, %s, %s, (SELECT id FROM severities WHERE level = %s), %s)
                ON DUPLICATE KEY UPDATE description = VALUES(description),
                                        condition = VALUES(condition),
                                        severity_level_id = (SELECT id FROM severities WHERE level = %s),
                                        category = VALUES(category)
            """
            data = (name, description, condition, severity_level.lower(), category, severity_level.lower())
            await self._execute_query(query, data)
            await self.cnx.commit()
            print(f"[AlertSystem] Inserted/updated alert rule: {name}")
        except mysql.connector.Error as err:
            print(f"[AlertSystem] Error inserting/updating alert rule '{name}': {err}")

    def _load_and_insert_alert_rules(self):
        engine = DetectionEngine()
        severity_map = {item['level'].lower(): item['id'] for item in self.fetch_severities()}

        async def insert_rules():
            for name, rule in engine.signature_rules.items():
                description = rule.get('details', f"Signature rule for {name}")
                severity = rule.get('severity', 'medium')  # Default to medium
                condition = "Defined in Python logic"  # We can't directly store a lambda
                category = rule.get('category')

                await self._insert_alert_rule(name, description, condition, severity, category)
